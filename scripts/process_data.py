...
def main():
 ...
 # Production precomputation for supported intervals.
 ...
 for interval in (15,20,25,30):
  ...
  for x in demand:
   eligible=(_date.fromisoformat(x["base_last"])+_td(days=interval)).isoformat()
  ...
  for d,drugs in sorted(day.items()):
   ...
   fn=f"demand-{d}.json";(pre/fn).write_text(json.dumps({"date":d,"rows":rows},ensure_ascii=False,separators=(",",":")),encoding="utf-8");didx[d]=f"data/precomputed/{interval}/{fn}"
